...
void FFTW_R2C_2D_Executor::execute() { fftw_execute(plan); }
...
//-----------------------------------------------------------------
 ...
 FFTW_C2R_2D_Executor::FFTW_C2R_2D_Executor(int n0_complex_samples, int n1_complex_samples) :
input_size_0(n0_complex_samples), input_size_1(n1_complex_samples),
input_buffer(fftw_alloc_complex(n0_complex_samples * n1_complex_samples)),
output_size_0(n0_complex_samples), output_size_1(n1_complex_samples),
output_buffer(fftw_alloc_real(n0_complex_samples * n1_complex_samples))
{
    plan = fftw_plan_dft_c2r_2d(input_size_0, input_size_1, input_buffer, output_buffer, FFTW_MEASURE);
    std::fill(output_buffer, output_buffer + (output_size_0 * output_size_1), 0.0);
    fftw_complex temp = {0.0, 0.0};
    std::memset(input_buffer, 0, sizeof(fftw_complex) * input_size_0 * input_size_1);
}
FFTW_C2R_2D_Executor::~FFTW_C2R_2D_Executor() 
{
    fftw_free(input_buffer);
    fftw_free(output_buffer);
    fftw_destroy_plan(plan);
...
void FFTW_C2R_2D_Executor::set_input(const std::complex<double>* buffer, int n0, int n1) {
    assert(n0 <= input_size_0);
    assert(n1 <= input_size_1);

    for (int i = 0; i < n1; ++i)
    {
        for (int j = 0; j < n0; ++j)
        {
            input_buffer[i * n0 + j][0] = buffer[i * n0 + j].real();
            input_buffer[i * n0 + j][1] = buffer[i * n0 + j].imag();
        }
    }


}
void FFTW_C2R_2D_Executor::set_input(const std::vector<std::complex<double> *> & vec, int rowSize) {
     assert(rowSize <= input_size_0);
     assert(vec.size() <= input_size_1);
     for (size_t i = 0; i < vec.size(); ++i)
     {
        ...
        for (size_t j = 0; j < rowSize; ++j)
        {
            input_buffer[i * rowSize + j][0] = row[i * rowSize + j].real();
            input_buffer[i * rowSize + j][1] = row[i * rowSize + j].imag();
        }
    }

}
void FFTW_C2R_2D_Executor::execute() { fftw_execute(plan); }

vector<double> FFTW_C2R_2D_Executor::get_output() {
    return vector<double>(output_buffer, output_buffer + (output_size_0 * output_size_1));
